...
async function initModule() {
  const factory = require("../out.js");
  var Module: MyEmscriptenModule = await factory();
  Module.ccall("SetMaxThreads", null, ["number"], [0]);
  return Module;
}

export async function SolveBoardPBN(dealPbn: DealPbn): Promise<FutureTricks> {
  ...
  try {
    ...
    for (let i = 0; i < futureTricks.cards; i++) {
      ...
      futureTricks.equals.push(
        Module.getValue(futp + futureTricksEqualsOffset + sizeOfInt * i, "i32")
      );
